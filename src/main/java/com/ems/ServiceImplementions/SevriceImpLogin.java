...
import com.ems.Repo.Reprologin;
...
public class SevriceImpLogin implements Servicelogin {
    ...
    @Override
    public boolean saveUsernameAndPassword(EntityloginDTO entityDTO) {
    ...
    public  ResponseEntity<Boolean> login(String username, String password) {
        ...
        if (employee.isPresent()) {
            ...
            if (emp.getPassword().equals(password)) {
                ...
                return ResponseEntity.ok(isSuperAdmin); // true for superadmin, false for admin
            }
        }
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(false); // Unauthorized login
    }
